...
public class InterfaceMethodref extends MemberRef {
	...
	public InterfaceMethodref(ConstantPool pool, RTConstantPool rtPool, InterfaceMethodrefConstantInfo info) {
		super(pool, rtPool);
	...
	public synchronized Method resolvedInterfaceMethod() throws ClassNotFoundException, IOException {
		if (method == null)
			method = resolveInterfaceMethod();
		return method;
	}

	private Method resolveInterfaceMethod() throws ClassNotFoundException, IOException {
		ClassInfo from = pool.getClassInfo();
		ClassInfo to = resolvedClass();
		if (!to.isInterface())
			throw new IncompatibleClassChangeError();
		method = lookupInterfaceMethod(to);
		if (method == null)
			throw new NoSuchMethodError(to.getName() + "." + name);
		if (!method.isAccessiableFrom(from))
			throw new IllegalAccessError();
	...
	private Method lookupInterfaceMethod(ClassInfo to) {
		for (Method m : to.getMethods()) {
			if (m.getName().equals(name) && m.getDescriptor().equals(descriptor))
				return m;
		}
		return lookupMethodInInterfaces(to);
	}
}
