...
public class CommandLineResolverTest {
	...
	@Test
	public void parseStartClassName() {
		CommandLine cmd1 = resolver.parse(new String[] { "cn.yusei.Hello" });
		CommandLine cmd2 = resolver.parse(new String[] { "cn.yusei.Hello", "cn.yusei.ClassTest" });
		assertFalse(cmd1.isError());
		assertTrue(cmd2.isError());
		assertEquals("cn.yusei.Hello", cmd1.getStartClassName());
		assertEquals(CommandLineResolver.UNKNOWN_OPTION + "cn.yusei.ClassTest", cmd2.getErrorMessages().get(0));
	}
}
